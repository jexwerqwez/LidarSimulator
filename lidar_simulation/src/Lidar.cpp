...
Lidar::Lidar(rclcpp::Node *node, const std::string &name,
             const std::string &frame_id, const std::string &topic,
             double lidar_height, int num_lasers, double alpha_begin,
...
void Lidar::setPosition(const Position3D &pose) { position_ = pose; }
...
auto Lidar::scan(const std::vector<std::shared_ptr<Object>> &scene_objects)
  ...
  for (auto &lr : rays_) {
    ...
    Eigen::Vector3d origin{position_.position.x, position_.position.y,
