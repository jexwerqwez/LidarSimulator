...
TEST(StressTest, HighObjectDensity) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic",
              1.0, 64, -30.0, 30.0, 25.0, M_PI / 180.0);

  lidar.setPosition(Position3D{0, 0, 1.0, 0, 0, 0});
  std::vector<std::shared_ptr<Object>> objects;

  for (int i = 0; i < 3000; ++i) {
    objects.push_back(std::make_shared<Sphere>(Position3D{i * 0.1, 0, 0, 0, 0, 0}, 0.1));
  }
...
TEST(StressTest, HighFrequencyScanning) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic",
              1.0, 32, -30.0, 30.0, 20.0, M_PI / 180.0);
...
TEST(StressTest, LongTermStability) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic",
              1.0, 16, -20.0, 20.0, 15.0, M_PI / 180.0);
...
TEST(StressTest, LidarColdStartTime) {
  ...
  EXPECT_LT(ms, 11);  // запуск за 11 мс
}
