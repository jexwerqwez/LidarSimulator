...
TEST(LidarTest, GeneratesNonEmptyPointCloud) {
  ...
  Lidar lidar(node.get(), "test_lidar", "test_frame", "test_topic", 1.0, 16,
              -30.0, 10.0, 20.0, M_PI / 180.0);
...
TEST(LidarTest, DetectsMultipleObjects) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic", 1.0, 32, -45.0, 45.0, 25.0,
              M_PI / 90.0);
  ...
  std::vector<std::shared_ptr<Object>> scene = {
      ...
      std::make_shared<Cylinder>(Position3D{-1.0, 2.0, 0.0, 0, 0, 0}, 1.0,
                                 1.0)};
...
TEST(LidarTest, IgnoresObjectsOutsideRange) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic", 1.0, 32, -45.0, 45.0, 3.0,
              M_PI / 180.0);
...
TEST(LidarTest, DetectsMovedObject) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic", 1.0, 16, -20.0, 20.0, 10.0,
              M_PI / 90.0);
...
TEST(LidarTest, DetectsLayeredPlanes) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic", 1.0, 20, -30.0, 30.0, 15.0,
              M_PI / 180.0);
...
TEST(LidarTest, RunsWithinReasonableTime) {
  ...
  Lidar lidar(node.get(), "lidar", "frame", "topic", 1.0, 64, -45.0, 45.0, 30.0,
              M_PI / 360.0);
...
TEST(LidarFunctionalTest, SingleSphereHit) {
  ...
  Lidar lidar(node.get(), "test_lidar", "test_frame", "test_topic", 1.0, 5,
              -10.0, 10.0, 15.0, M_PI / 60.0);
...
TEST(LidarFunctionalTest, NearestObjectIsDetected) {
  ...
  Lidar lidar(node.get(), "test_lidar", "test_frame", "test_topic", 1.0, 5,
              -10.0, 10.0, 20.0, M_PI / 60.0);

  auto near_obj =
      std::make_shared<Sphere>(Position3D{2.0, 0.0, 1.0, 0, 0, 0}, 0.5);
  auto far_obj =
      std::make_shared<Sphere>(Position3D{6.0, 0.0, 1.0, 0, 0, 0}, 0.5);
...
TEST(LidarFunctionalTest, LidarOrientationChangesScan) {
  ...
  Position3D pose1{0, 0, 1.0, 0, 0, 0};
  Position3D pose2{0, 0, 1.0, 0, M_PI / 6, 0};
