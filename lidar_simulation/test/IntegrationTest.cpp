...
TEST(ROS2IntegrationTest, LidarPublishesPointCloud) {
  ...
  Lidar lidar(node.get(), "test_lidar", "test_frame", "test_topic",
              1.0, 10, -30.0, 30.0, 15.0, M_PI / 180.0);
...
TEST(PCLIntegrationTest, VoxelGridReducesPointCount) {
  ...
  Lidar lidar(node.get(), "test", "frame", "topic", 1.0, 20, -45, 45, 30.0, M_PI / 180.0);
  lidar.setPosition(Position3D{0, 0, 1, 0, 0, 0});
...
TEST(StressTest, ComplexObjectMotion) {
  ...
  Lidar lidar(node.get(), "motion_lidar", "frame", "topic", 1.0, 32, -30, 30, 50.0, M_PI / 180.0);
  lidar.setPosition(Position3D{0, 0, 1, 0, 0, 0});

  std::vector<std::shared_ptr<Object>> scene;
  for (int i = 0; i < 100; ++i) {
    scene.push_back(std::make_shared<Sphere>(Position3D{0.1 * i, 0.1 * i, 1.0, 0, 0, 0}, 0.2));
  }
...
TEST(StartupPerformanceTest, StartupCompletesQuickly) {
  ...
  Lidar lidar(node.get(), "startup_lidar", "frame", "topic", 1.0, 10, -10, 10, 10.0, M_PI / 90.0);
  lidar.setPosition(Position3D{0, 0, 1, 0, 0, 0});
  ...
  EXPECT_LT(time_ms, 300);  // запуск + сканирование должны уложиться в 300 мс
}
