...
PointCloudPtr CloudMergerNode::removeGroundPlane(const PointCloudPtr &cloud) {
  ...
  seg.setDistanceThreshold(0.1);  // допуск в метрах до плоскости
...
void CloudMergerNode::cloudCallback(
    size_t idx, sensor_msgs::msg::PointCloud2::SharedPtr msg) {
  PointCloudT::Ptr pc(new PointCloudT());
  pcl::fromROSMsg(*msg, *pc);
  RCLCPP_INFO(this->get_logger(),
              "cloud[%zu] received with %lu points. Sample intensity: %.2f",
...
void CloudMergerNode::publishMergedCloud() {
  ...
  for (bool r : received_) {
    if (!r) {
      RCLCPP_DEBUG(get_logger(), "waiting for all %zu clouds",
                   received_.size());
      return;
    }
  }
...
PointCloudT::Ptr CloudMergerNode::mergeAll(
  ...
  // воксельная фильтрация ко всем облакам
  ...
  for (auto &c : clouds) {
    if (c && !c->empty()) {
      ...
      filtered = removeGroundPlane(filtered);  // удаляем плоскость пола
...
DescriptorPtr CloudMergerNode::computeDescriptors(
    const PointCloudPtr &keypoints, const PointCloudPtr &surface) {
  // вычисляем нормали
...
PointCloudPtr CloudMergerNode::alignClouds(const PointCloudPtr &source,
  ...
  // трансформируем исходное облако и склеиваем
